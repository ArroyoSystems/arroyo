...
use datafusion::functions_aggregate::{
    average, correlation, count, covariance, median, regr, stddev, sum, variance,
};
...
/// These implementations can be rebuilt after exporting mutable state. Other
/// sliding accumulators keep the legacy checkpoint path and remain in place.
/// supports_retract_batch() still determines sliding vs. stored-input placement.
pub(super) fn state_fields(expr: &AggregateFunctionExpr) -> Result<Option<Vec<FieldRef>>> {
    ...
    let supported = is_sum
        ...
        || udf.downcast_ref::<median::Median>().is_some()
...
mod tests {
    ...
    #[test]
    fn median_checkpoint_preserves_inputs_for_restore() -> Result<()> {
        continuation(
            median::median_udaf(),
