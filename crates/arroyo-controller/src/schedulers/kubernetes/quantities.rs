...
mod tests {
    ...
    fn test_parse_memory_m() {
        ...
        assert_eq!(q.parse_memory().unwrap().value(), 1_000_000_000);
    }

    #[test]
    fn test_parse_memory_g() {
        let q = Quantity("1G".to_string());
        assert_eq!(q.parse_memory().unwrap().value(), 1_000_000_000_000);
    }

    #[test]
    fn test_parse_memory_t() {
        let q = Quantity("1T".to_string());
        assert_eq!(q.parse_memory().unwrap().value(), 1_000_000_000_000_000);
    }
    ...
    fn test_parse_memory_mi() {
        let q = Quantity("1Mi".to_string());
        assert_eq!(q.parse_memory().unwrap().value(), 1_048_576_000);
    }

    #[test]
    fn test_parse_memory_gi() {
        let q = Quantity("1Gi".to_string());
        assert_eq!(q.parse_memory().unwrap().value(), 1_073_741_824_000);
    }

    #[test]
    fn test_parse_memory_ti() {
        let q = Quantity("1Ti".to_string());
        assert_eq!(q.parse_memory().unwrap().value(), 1_099_511_627_776_000);
    }

    #[test]
    fn test_parse_memory_pi() {
        let q = Quantity("1Pi".to_string());
        assert_eq!(q.parse_memory().unwrap().value(), 1_125_899_906_842_624_000);
    }
