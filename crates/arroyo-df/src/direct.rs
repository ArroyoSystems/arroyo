...
use crate::{CompiledSql, WindowBehavior};
...
impl PlanToGraphVisitor {
    ...
    pub fn into_compiled(self) -> CompiledSql {
        CompiledSql {
            ...
            // TODO: populate these or remove them.
            connection_ids: vec![],
            schemas: HashMap::new(),
        }
    }
    pub fn build_source(&mut self, table_source: &TableSourceExtension) -> DFResult<()> {
        ...
        let sql_source = table_source
            .table
            .as_sql_source()
            .map_err(|e| DataFusionError::Plan(format!("as_sql_source error: {}", e)))?;
