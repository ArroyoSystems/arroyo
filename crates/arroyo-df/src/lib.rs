...
use datafusion_expr::{
    AccumulatorFactoryFunction, Expr, Extension, LogicalPlan, ReturnTypeFunction,
    ScalarFunctionDefinition, ScalarUDF, Signature, StateTypeFunction, Volatility, WindowUDF,
};

use datafusion_expr::{AggregateUDF, TableSource};
use logical::LogicalBatchInput;

use schemas::{add_timestamp_field_if_missing_arrow, window_arrow_struct};

use tables::{Insert, Table};

use crate::direct::PlanToGraphVisitor;
...
pub async fn parse_and_get_arrow_program(
    ...
    Ok(plan_to_graph_visitor.into_compiled())
}
