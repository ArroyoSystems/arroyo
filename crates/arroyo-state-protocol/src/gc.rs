...
use crate::store::{read_protobuf, ProtocolStore, StoreError};
...
{
    let history = collect_history_and_clean_checkpoint_files(store, paths, head, new_min_epoch)
        .await?;
    ...
    for c in history {
        ...
        store.delete_object(&paths.checkpoint_manifest(c.generation, c.epoch)).await?;
    }
    ...
    S: ProtocolStore + ?Sized,
{
    ...
    while let Some(checkpoint_ref) = next {
        ...
        let Some(manifest): Option<CheckpointManifest> = read_protobuf(store, &checkpoint_ref).await? else {
            if history.is_empty() {
        ...
        next = manifest.parent_checkpoint_ref.map(|r| CheckpointRef::new(r)).transpose()?;
    }
    ...
    S: ProtocolStore + ?Sized,
{
    ...
    for operator in &checkpoint.operators {
        for (table_name, metadata) in &operator.table_checkpoint_metadata {
            let op_metadata = operator.operator_metadata.as_ref()
                .ok_or_else(|| {
            ...
            table_checkpoint_data_files(&op_metadata.operator_id, table_name, manifest_path, metadata, &mut to_delete)?;
        }
    }

    to_delete.push(paths.committed_marker(Generation(checkpoint.generation), Epoch(checkpoint.epoch)));
    ...
    match metadata.table_type() {
        TableEnum::MissingTableType => {
            return Err(StoreError::InvalidProtobuf {
                ...
                msg: format!("table metadata for operator '{}' table '{}' is missing table type",
        ...
        TableEnum::ExpiringKeyedTimeTable => {
            return Err(StoreError::InvalidProtobuf {
                path: metadata_path.clone(),
                msg: format!("table metadata for operator '{}' table '{}' has table type \
