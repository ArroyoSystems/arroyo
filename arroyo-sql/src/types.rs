...
use arroyo_rpc::types::{
    Format, JsonFormat, PrimitiveType, SourceField, SourceFieldType, StructType, TimestampFormat,
};
...
impl From<SourceField> for StructField {
    fn from(f: SourceField) -> Self {
        let t = match f.field_type {
            SourceFieldType::Primitive(pt) => TypeDef::DataType(
                match pt {
            ...
            SourceFieldType::Struct(s) => TypeDef::StructDef(
                StructDef::for_name(
                    s.name.clone(),
