...
use crate::rest_types::{Job, JobCollection, Pipeline, PipelineCollection, PipelinePost};
...
use crate::{handle_db_error, log_and_map, optimizations, required_field, types::public, AuthData};
...
impl Into<Pipeline> for DbPipelineRest {
    fn into(self) -> Pipeline {
        Pipeline {
            ...
            udfs: serde_json::from_value(self.udfs).unwrap(),
        }
    }
}
...
impl Into<Job> for DbPipelineJob {
    fn into(self) -> Job {
        Job {
            id: self.id,
            running_desired: self.stop == public::StopMode::none,
...
pub async fn post_pipeline(
    ...
    Json(pipeline_post): Json<PipelinePost>,
...
pub async fn get_pipelines(
    ...
    let pipelines: Vec<DbPipelineRest> = api_queries::get_pipelines()
