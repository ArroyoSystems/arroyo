...
use arroyo_rpc::grpc::{
    api::api_grpc_server::ApiGrpcServer,
};
use arroyo_server_common::{log_event, start_admin_server};
use arroyo_types::{
    API_ENDPOINT_ENV, ASSET_DIR_ENV, CONTROLLER_ADDR_ENV, DatabaseConfig, grpc_port, HTTP_PORT_ENV,
    ports, service_port, telemetry_enabled,
};
...
async fn server(pool: Pool) {
    ...
    let app = rest::create_rest_app(server.clone());
